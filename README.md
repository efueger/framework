...
Controllers can use a [configuration](https://github.com/mvc5/framework/blob/master/src/Config/Configuration.php) object as a model for the view to use which also includes the template file name and an optional child model which is used by the [layout model](https://github.com/mvc5/framework/blob/master/src/View/Layout/LayoutModel.php). For convenience, controllers can use an existing model trait that has methods for setting the model and returning it. If no model is injected then a new instance of a standard model will be created and returned. When a controller is invoked and returns a model, it is stored as the content of the response object and will be rendered prior to sending the response.

The model trait used by the controller has two methods named [model](https://github.com/mvc5/framework/blob/master/src/View/Model/Service/ViewModel.php#L28) and [view](https://github.com/mvc5/framework/blob/master/src/View/Model/Service/ViewModel.php#L44). This allows the controller to choose to use the view method when a specific template is to be used, or alternatively the controller can use the model method and pass an array variables as its configuration.
